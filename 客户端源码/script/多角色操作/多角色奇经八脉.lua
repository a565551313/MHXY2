...
-- @Last Modified time: 2025-06-07 16:53:48
--======================================================================--
local 多角色奇经八脉 = class()
...
local function 取重复技能介绍(名称,门派)
    if 名称 == "不灭" then
         if 门派=="魔王寨" then
          return "秘传法术造成伤害提升10%，且使目标直接阵亡的几率提升2%。"
        elseif 门派=="方寸山"  then
...
self.定位建议={}
  ...
  for n=2,5 do
      self.资源组[n]:绑定窗口_(121)

  end

  for n=1,3 do
     self.切换按钮[n]:绑定窗口_(121)
  ...
  self.资源组[5]:显示(self.x+460,self.y+441,true)
    ...
    if self.查看流派 ~= self.数据.经脉流派 then
       ...
       self:刷新查看()
        ...
        if self.选中~=nil then
         请求服务(63,{参数=self.玩家id,文本="经脉加点",序列=self.选中})
        self.选中 = nil
       end
    end
	...
	elseif self.资源组[5]:事件判断() then --清空
       ...
       tp.窗口.对话栏:文本(tp.多角色[self.玩家id].模型,tp.多角色[self.玩家id].名称, "请选择重置经脉",{"重置该角色当前经脉","重置该角色全部经脉"})
    end
  elseif self.切换流派按钮:事件判断() then
        local 事件 = function()
         请求服务(63,{参数=self.玩家id,文本="切换经脉",经脉=self.查看流派})
        end
	...
	for i=1,3 do
		self.切换按钮[i]:更新(x,y)
    self.切换按钮[i]:显示(self.x-175+i*200,self.y+25,true,nil,nil,self.切换按钮[i].按钮文字 == self.查看流派,2)
    if self.切换按钮[i].按钮文字 == self.数据.经脉流派 then
      self.当前图片:显示(self.x-175+i*200,self.y+25)
    end
			...
			for i=1,3 do
              ...
              self.资源组[6][m].介绍=jj.介绍
                if qj[m]=="不灭" or qj[m]=="感念" or qj[m]=="静心" or qj[m]=="矫健"
                  ...
                  or qj[m]=="混元" or qj[m]=="聚魂" or qj[m]=="救主" or qj[m]=="灵能" then
