...
-- @Last Modified time: 2024-11-17 19:53:45
...
function 动画类:初始化(wj,文件号,p,len)
    ...
    if 全局资源缓存[self.path] then --5分钟缓存
        全局资源缓存[self.path].time = os.time() + 30
...
function 动画类:置染色(id,a,b,c,d)--新版本修改
    if id == nil then
        return
    end
    if id == "黑白" then
    ...
    if self.was.Palette then
        ...
        self:调色(a or 0,b or 0,c or 0,0)
        self:重置纹理()
        self:置方向(0)
    end
end

function 动画类:调色(...)--新版本修改
    local v = {...}
    local n,p = 0
    for i=0,255 do
