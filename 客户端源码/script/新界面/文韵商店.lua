...
--======================================================================--
local 场景类_文韵商店 = class()
local tp,zts,zts1
local insert = table.insert
local xxx = 0
local yyy = 0
local sts = {"单价","数量","总额","积分"}
local tos = 取金钱颜色
local mouseb = 引擎.鼠标弹起
function 场景类_文韵商店:初始化(根)
	self.ID = 148
	self.x = 364
	self.y = 100
	self.xx = 0
	...
	local 总控件 = self.控件类:创建控件('文韵商店总控件')
	总控件:置可视(true,true)
	self.输入框 = 总控件:创建输入("文韵商店数量输入",0,0,100,14)
...
function 场景类_文韵商店:加载数据()	-- body
	...
	self.资源组 = {
		...
		[3] = 按钮.创建(自适应.创建(12,4,55,20,1,3),0,0,4,true,true,"购买"),
		[4] = 资源:载入('登陆资源.wdf',"网易WDF动画",0xE9C090A3),
		[5] = 自适应.创建(3,1,94,18,1,3),
		[6] = 自适应.创建(1,1,268,18,1,3,nil,18),
	...
	local 物品格子 = tp._物品格子
	for i=1,20 do
		self.商品[i] = 物品格子(0,i,n,"文韵商店")
	end
end

function 场景类_文韵商店:显示(dt,x,y)
	self.资源组[2]:更新(x,y)
	...
	elseif self.资源组[3]:事件判断() then
		...
		请求服务(1503.3,{数量=self.数量,商品=self.道具.原始商品})
	end
	self.焦点 = false
	self.资源组[1]:显示(self.x,self.y)

	tp.窗口标题背景_[4]:显示(self.x+self.资源组[1].宽度/2-60,self.y+2)
  	zts1:置颜色(白色):显示(self.x+self.资源组[1].宽度/2-30,self.y+2,"文韵商店")
	zts:置颜色(4294967295)
	zts:置字间距(15)
	for i=0,3 do
	...
	self.资源组[3]:显示(self.x+195,self.y+321+24+24+21,true)
	local xx = 0
	local yy = 0
	for i=1,20 do
		self.商品[i]:置坐标(self.x + xx * 51 + 9,self.y + yy * 51 + 25)
	...
	if self.道具 ~= nil then
		zts:置颜色(-16777216)
		zts:显示(self.x + 80,self.y + 321,self.单价)
		self.输入框:置坐标(self.x + 80,self.y + 321+24)
		...
		zts:显示(self.x + 80,self.y + 321+24+24,(self.数量 * self.单价))
	end
	zts:置颜色(tos(tp.金钱))
	zts:显示(self.x + 80,self.y + 321+24+24+24,tp.金钱)
	zts:置颜色(白色)
	self.控件类:更新(dt,x,y)
...
function 场景类_文韵商店:检查点(x,y)
	if self.资源组 ~= nil and self.资源组[1]:是否选中(x,y)  then
		return true
	end
end

function 场景类_文韵商店:初始移动(x,y)
	tp.运行时间 = tp.运行时间 + 1
