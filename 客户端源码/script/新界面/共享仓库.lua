...
function 共享仓库:打开(道具,道具仓库总数,召唤兽仓库总数)
	if self.可视 then
		self.仓库开始 = 1
		self.仓库结束 = 100
		self.选中仓库 = nil
...
function 共享仓库:刷新召唤兽仓库总数(总数,仓库数据)
	self.仓库类型="召唤兽"
 	self.召唤兽仓库总数 = 总数
 	self.仓库页数 = 1
 	self.背包页数 = 1
...
function 共享仓库:显示(dt,x,y)
	...
	self.资源组[12]:更新(x,y)
  	...
  	elseif self.资源组[5]:事件判断() then
  		self.道具类型 = "道具"
        请求服务(202,{文本="获取道具",道具类型=self.道具类型})
	...
	if self.仓库类型 == "道具" then
		...
		elseif self.资源组[14]:事件判断() then
			请求服务(202,{文本="整理仓库",道具类型=self.道具类型,页数=self.仓库页数})
				...
				if self.物品[i].物品 ~= nil then
					...
					elseif 引擎.鼠标弹起(右键) and (分割名称[1] ~= "" and 分割名称[1] ~= "帮派银票") then
						...
						if self.选择物品 ~= nil then
				 		...
				 		请求服务(202,{文本="存入仓库",页数=self.仓库页数,物品=i,类型=self.道具类型})
				...
				if self.仓库[i].物品 ~= nil then
					...
					if self.仓库[i].事件 and self.鼠标 then
	               	...
	               	elseif 引擎.鼠标弹起(右键) then
					 	请求服务(202,{文本="取出物品",页数=self.仓库页数,物品=i,类型=self.道具类型})
					end
				end
			end
			xx = xx + 1
	...
	elseif self.仓库类型 == "召唤兽" then
		...
		for n=1,self.召唤兽仓库总数 do
	  	    if self.召唤兽仓库组[n]:事件判断() and self.召唤兽仓库组[n].事件 and self.召唤兽仓库组[n].窗口按钮 then
				self.仓库页数 = n
				请求服务(202,{文本="获取召唤兽",序列=self.仓库页数})
			end
	  	end
		...
		self.资源组[4]:显示(self.x + 195,self.y + 300,true)
  		if #bblb > 7 then
			self.资源组[19]:置高度(math.min(math.floor(143/(#bblb-7)),104))
			self.召唤兽起始 = math.min(math.ceil((#bblb-7)*self.资源组[19]:取百分比()),#bblb-7)
		end
  		self.资源组[16]:更新(x,y,self.召唤兽起始 > 0)
		self.资源组[17]:更新(x,y,self.召唤兽起始 < #bblb - 7)
		...
		elseif self.资源组[11]:事件判断() then
			if self.选择召唤兽~=0 and bblb[self.选择召唤兽]~=nil then
				if bblb[self.选择召唤兽].参战信息~=nil then
					...
					if #bblb > 7 then
						self.资源组[19]:置起始点(self.资源组[19]:取百分比转换(self.召唤兽起始-1,7,#bblb))
					end
					请求服务(202,{文本="存入召唤兽",认证码=self.选择召唤兽认证码,页数=self.仓库页数})
				end
			else
		...
		elseif self.资源组[12]:事件判断() then
			if self.选择仓库召唤兽~=0 and self.召唤兽仓库数据[self.选择仓库召唤兽]~=nil then
				...
				请求服务(202,{文本="取出召唤兽",认证码=self.选择仓库召唤兽认证码,页数=self.仓库页数})
			else
		...
		if bblb[1]~=nil and bblb[self.选择召唤兽] ~= nil and self.选择召唤兽-self.召唤兽起始 <= 7 then
