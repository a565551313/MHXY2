...
function 藏宝阁出售寄存:初始化(根)
    ...
    self.资源组={
        ...
        [5] = 自适应.创建(3,1,130,18,1,3),
        [6] = 按钮.创建(自适应.创建(13,4,45,23,1,3),0,0,4,true,true,"上架"),
        [7] = 按钮.创建(自适应.创建(13,4,45,23,1,3),0,0,4,true,true,"取回"),
    ...
    self.资源组[6]:绑定窗口_(115)
    self.资源组[7]:绑定窗口_(115)
    for i=1,5 do
        self.道具按钮[i]:绑定窗口_(115)
    end
    self.boxh=40
    self.boxw=225
    self.数据={}
    ...
    local 总控件 = self.控件类:创建控件('藏宝阁寄存上架商品')
    总控件:置可视(true,true)
    self.输入框 = 总控件:创建输入("藏宝阁寄存上架输入",0,0,126,16)
    ...
    if self.可视 then
        ...
        insert(tp.窗口_,self)
         if self.类型 ==1 then
            ...
            self.额外数据[i] = {}
             if self.数据.额外数据[i]~=nil then
                self.额外数据[i] = self.数据.额外数据[i]
                self.额外数据[i].识别码 = self.数据.数据[i].识别码
            end
           end
            ...
            for n=1,#self.数据.数据 do
                ...
                self.额外数据[n] = {}
                if self.数据.额外数据[n]~=nil then
                    self.额外数据[n] = self.数据.额外数据[n]
                end
            end
       end
        tp.运行时间 = tp.运行时间 + 1
...
function 藏宝阁出售寄存:显示(dt,x,y)
    ...
    zts1:置颜色(白色)
    zts1:置字间距(3)
    zts1:显示(self.x+self.资源组[1].宽度/2-38,self.y+2,"藏宝阁寄存")
    zts1:置字间距(0)
    self.资源组[2]:更新(x,y)
    ...
    elseif self.资源组[6]:事件判断()  then---------上架按钮
        if  self.资源组[6]:取文字() =="上架" then
            if tonumber(self.输入框:取文本()) == nil  then
                tp.常规提示:打开("#Y/请正确输入出售价格！")
            elseif self.商城类型 =="" or self.商城类型 == nil  then
                tp.常规提示:打开("#Y/选择类型错误！")
            elseif self.识别码 =="" or self.识别码 == nil  then
                tp.常规提示:打开("#Y/选择类型错误！")
             else
                请求服务(69,{类型=self.商城类型,文本="上架寄存商品",编号=self.识别码,价格=self.输入框:取文本()})
               self.资源组[6]:置文字("下架")
             end
        else
            if self.商城类型 =="" or self.商城类型 == nil  then
                tp.常规提示:打开("#Y/选择类型错误！")
            elseif self.识别码 =="" or self.识别码 == nil  then
                tp.常规提示:打开("#Y/选择类型错误！")
            else
               请求服务(69,{类型=self.商城类型,文本="下架商品",编号=self.识别码})
               self.资源组[6]:置文字("上架")
            end
        end
        elseif self.资源组[7]:事件判断()  then
            if self.额外数据[self.选中] ==nil then
                  tp.常规提示:打开("#Y/选择类型错误！")
            elseif self.额外数据[self.选中].上架 then
                  tp.常规提示:打开("#Y/商品上架中无法取回！")
            elseif self.商城类型 =="" or self.商城类型 == nil  then
                tp.常规提示:打开("#Y/选择类型错误！")
            elseif self.识别码 =="" or self.识别码 == nil  then
                tp.常规提示:打开("#Y/选择类型错误！")
            else
               请求服务(69,{类型=self.商城类型,文本="取回商品",编号=self.识别码,类别="上架"})
            end
    ...
    if self.类型 == 1 then
           ...
           self.资源组[6]:显示(self.x+190,self.y+275)
           self.资源组[7]:显示(self.x+240,self.y+275)
           ...
           for i=1,5 do
            ...
            for i=self.开始,self.结束 do
                ...
                self.物品[i]:显示(dt,x,y,self.鼠标)
                  if self.额外数据[i]~=nil then
                     if self.额外数据[i].上架 then
                        zts:置颜色(红色):显示(25 + self.x+xx*51,38 + self.y+yy*51,"上架中")
                    end
                 end
                if self.物品[i].焦点 then
                    if self.物品[i].物品 ~= nil then
                        ...
                        if self.物品[i].事件 then
                            ...
                            self.选中 = i
                             if self.额外数据[i]~=nil then
                                self.识别码 =self.额外数据[i].识别码
                                self.商城类型 =self.额外数据[i].商城类型
                                if self.额外数据[i].上架 then
                                    self.资源组[6]:置文字("下架")
                                    self.输入框:置文本(self.额外数据[i].价格)
                                else
                                    self.资源组[6]:置文字("上架")
                                     self.输入框:置文本("")
                                end
           ...
           self.资源组[6]:显示(self.x+190,self.y+295)
           self.资源组[7]:显示(self.x+240,self.y+295)
            ...
            for n=self.开始,self.结束 do
                if self.头像组[n]~=nil then
                    ...
                    if bw:检查点(x,y) then
                        if mouseb(0) then
                            self.选中 = n
                            self.识别码 =self.数据.数据[n].识别码
                             if self.额外数据[n]~=nil then
                                self.商城类型 ="宝宝商城"

                                if self.额外数据[n].上架 then
                                    self.资源组[6]:置文字("下架")
                                    self.输入框:置文本(self.额外数据[n].价格)
                    ...
                    if self.选中 == n then
                     ...
                     if self.额外数据[n]~=nil then
                        if self.额外数据[n].上架 then
                            zts:置颜色(红色):显示(zx+45,zy+5,"姓名:"..self.数据.数据[n].名称)
                            zts:置颜色(红色):显示(zx+45,zy+25,"等级:"..self.数据.数据[n].等级)
                            zts:置颜色(红色):显示(zx+145,zy+25,"种类:"..self.数据.数据[n].种类)
                        else
...
function 藏宝阁出售寄存:开始移动(x,y)
    if self.鼠标 then
        self.x = x - self.xx
        self.y = y - self.yy
    end
end
function 藏宝阁出售寄存:更新(dt) end
