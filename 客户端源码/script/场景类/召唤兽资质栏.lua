...
-- @Last Modified time: 2025-04-07 15:00:36
--======================================================================--
...
function 场景类_召唤兽资质栏:刷新数据(b)
	...
	self.超级赐福={}
	for i=1,4 do
		if bb.超级赐福~=nil and bb.超级赐福[i]~=nil then
		   local 临时技能=tp._技能()
		   临时技能:置对象(bb.超级赐福[i])
		    self.赐福技能[i]:置技能(临时技能,true)
		    self.超级赐福[bb.超级赐福[i]]=true
		end
	end
    local 已有技能={}
	for i=1,#bb.技能 do
	    self.技能[i]:置技能(bb.技能[i])
	    if self.技能[i].技能~=nil and self.技能[i].技能.名称~=nil and self.超级赐福[self.技能[i].技能.名称] then
			已有技能[self.技能[i].技能.名称]=true
		end
	end
	...
	self.技能页数 = 1
	for i=1,6 do
		if bb.内丹数据[i] ~= nil and bb.内丹数据[i].技能 ~= nil then
			self.内丹数据[i] =bb.内丹数据[i]
			item = self:取内丹数据(bb.内丹数据[i].技能,bb.内丹数据[i].等级)
...
function 场景类_召唤兽资质栏:加载资源()
   ...
   self.技能资源组 ={
	    			[1] = 自适应.创建(0,1,210,100,3,9),
	    			[2] = 自适应.创建(1,1,206,18,1,3,nil,18),--标题背景
	    			[3] = 按钮.创建(自适应.创建(18,4,16,16,4,3),0,0,4,true,true),--关闭
			}
   	self.超级图标=资源:载入(wdf配置.."/pic/超级兽决技能小图标.png","图片")
   	self.超级按钮=资源:载入(wdf配置.."/pic/超级兽决技能小图标.png","图片")
...
function 场景类_召唤兽资质栏:显示(dt,x,y)
	...
	zts1:置字间距(3)
	zts1:显示(self.x+self.资源组[1].宽度/2-38,self.y+2,"召唤兽资质")
	zts1:置字间距(0)
    ...
    if bb.饰品~=nil and bb.饰品攻资~=nil then
     zts:置颜色(黄色)
	 zts:显示(self.x+165,self.y+100,"+"..bb.饰品攻资)
	 zts:置颜色(-16777216)
       end
	zts:显示(self.x+131,self.y+123,bb.防御资质)
	 if bb.饰品~=nil and bb.饰品防资~=nil then
     zts:置颜色(黄色)
	 zts:显示(self.x+165,self.y+123,"+"..bb.饰品防资)
	 zts:置颜色(-16777216)
       end
	zts:显示(self.x+131,self.y+146,bb.体力资质)
	 if bb.饰品~=nil and bb.饰品体资~=nil then
     zts:置颜色(黄色)
	 zts:显示(self.x+165,self.y+146,"+"..bb.饰品体资)
	 zts:置颜色(-16777216)
       end
	zts:显示(self.x+131,self.y+169,bb.法力资质)
	 if bb.饰品~=nil and bb.饰品法资~=nil then
     zts:置颜色(黄色)
	 zts:显示(self.x+165,self.y+169,"+"..bb.饰品法资)
	 zts:置颜色(-16777216)
       end
	zts:显示(self.x+131,self.y+192,bb.速度资质)
	 if bb.饰品~=nil and bb.饰品速资~=nil then
     zts:置颜色(黄色)
	 zts:显示(self.x+165,self.y+192,"+"..bb.饰品速资)
	 zts:置颜色(-16777216)
       end
	zts:显示(self.x+131,self.y+215,bb.躲闪资质)
	 if bb.饰品~=nil and bb.饰品躲资~=nil then
     zts:置颜色(黄色)
	 zts:显示(self.x+165,self.y+215,"+"..bb.饰品躲资)
	 zts:置颜色(-16777216)
       end
   	if bb.种类=="神兽" then
	...
	if self.状态 == 1 then
		if  bb.超级赐福~=nil then
			self.超级按钮:显示(self.x+190,self.y+310)
			  if self.超级按钮:是否选中(x,y) and 引擎.鼠标弹起(0)  then--鼠标上滚
				    self.技能显示=true
			  end
		end
        ...
        for n=1,self.显示数量 do
			...
			local jy = self.y+309+(self.a*41-41)
	   	    ...
	   	    if  self.技能[self.显示序列].技能 ~= nil and  self.技能[self.显示序列].技能.名称 ~= nil then
	   	    	if self.超级赐福[self.技能[self.显示序列].技能.名称] then
	   	    	   self.超级图标:显示(jx+5,jy+5)
	   	    	   if self.技能[self.显示序列].焦点 then
				   	   tp.提示:超级技能(x,y,self.技能[self.显示序列].技能.名称,"(已赐福)")
				   end
	   	    	else
	   	    	    if self.技能[self.显示序列].焦点 then
	   	...
	   	if  self.技能显示 then
				 self.技能资源组[1]:显示(self.x+15,self.y+200)
				 self.技能资源组[2]:显示(self.x+17,self.y+202)
				 self.技能资源组[3]:更新(x,y)
				 self.技能资源组[3]:显示(self.x+205,self.y+202)
				 tp.窗口标题背景_[4]:显示(self.x+15+self.技能资源组[1].宽度/2-68,self.y+202)
				 zts1:置字间距(3)
				 zts1:置颜色(白色):显示(self.x+15+self.技能资源组[1].宽度/2-30,self.y+202,"赐福效果")
				 zts1:置字间距(0)
				 if self.技能资源组[3]:事件判断()   then
				  ...
				  for n=1,4 do
					  if self.赐福技能[n]~=nil then
						   self.赐福技能[n]:置坐标(self.x+25+(n-1)*50,self.y+240)
						   self.赐福技能[n]:显示(x,y,self.鼠标)

						    if self.赐福技能[n].技能 ~= nil and  self.赐福技能[n].技能.名称 ~= nil then
						    	self.超级图标:显示(self.x+27+(n-1)*50,self.y+243)
						       if self.赐福技能[n].焦点 then
