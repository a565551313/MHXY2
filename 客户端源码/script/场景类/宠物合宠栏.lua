...
function 场景类_宠物合宠栏:加载资源()
	...
	local 物品 = tp._物品格子
	self.资源组 = {
		[0] = 自适应.创建(1,1,566,18,1,3,nil,18),
		[1] = 自适应.创建(0,1,570,450,3,9),
		[2] = 按钮.创建(自适应.创建(18,4,16,16,4,3),0,0,4,true,true),
		[3] = 按钮.创建(自适应.创建(12,4,55,20,1,3),0,0,4,true,true,"合宠"),
		[4] = 按钮.创建(自适应.创建(12,4,55,20,1,3),0,0,4,true,true,"选择"),
		[5] = 按钮.创建(自适应.创建(12,4,55,20,1,3),0,0,4,true,true,"选择"),
	...
	self.副上页 = 按钮.创建(资源:载入('登陆资源.wdf',"网易WDF动画",0x7AB5584C),0,0,4,true,true)
	self.副下页 = 按钮.创建(资源:载入('登陆资源.wdf',"网易WDF动画",0xCB50AB1D),0,0,4,true,true)
...
function 场景类_宠物合宠栏:显示(dt,x,y)
	...
	zts1:置颜色(白色):显示(self.x+self.资源组[1].宽度/2-38,self.y+3,"合  宠")
	...
	self.资源组[2]:显示(self.x + 549,self.y + 2)
    ...
    elseif self.资源组[3]:事件判断() then
    	...
    	if (os.time() - self.时间 > 1 or self.时间 == 0) then
    		self.时间 = os.time()
    		 local 事件 = function()
	          请求服务(5009,{序列=self.主选择认证,序列1=self.副选择认证})
	          self:清除()
             end
            tp.窗口.文本栏:载入("注:两个相同技能宝宝合出的宝宝技能会减少,有一个宝宝技能少的合出的宝宝技能会大幅度减少",nil,true,事件)
				...
				if 引擎.鼠标弹起(0) and self.鼠标 and not tp.消息栏焦点 then
					...
					for n=1,24 do
						...
						if bb.技能[n]~=nil and bb.技能[n].名称==bb.法术认证 then
							...
							self.副技能[序列]:置技能(bb.技能[n])
						end
					end
                    self.副内丹数据={}
					for n=1,#bb.内丹数据 do
						if bb.内丹数据[n]~=nil then
				         	self.副内丹数据[n] =bb.内丹数据[n]
							local item = self:取内丹数据(bb.内丹数据[n].技能,bb.内丹数据[n].等级)
