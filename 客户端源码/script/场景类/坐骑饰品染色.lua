...
function 场景类_坐骑饰品染色:加载数据()
	...
	self.资源组 = {
		...
		[2] = 按钮.创建(tp.资源:载入('登陆资源.wdf',"网易WDF动画",0xF11233BB),0,0,4,true,true),
		...
		[9] = 按钮.创建(tp.资源:载入('登陆资源.wdf',"网易WDF动画",0x01000026),0,0,4,true,true),
	}
	self.染色组={}
	for n=1,80 do
		self.染色组[n]=按钮.创建(自适应.创建(10,4,28,21,1,3),0,0,4,true,true,n)
	end
end
...
function 场景类_坐骑饰品染色:显示(dt,x,y)
	...
	tp.窗口标题背景_[6]:显示(self.x+self.资源组[1].宽度/2-75,self.y+2)
	zts1:显示(self.x+self.资源组[1].宽度/2-45,self.y+3,self.注释)
	...
	self.资源组[2]:显示(self.x + 440,self.y + 2)
	...
	if self.鼠标 then
		...
		elseif self.资源组[5]:事件判断() then
			请求服务(3758,{序列=bb.认证码,序列1=bb.饰品物件.染色方案,序列2=bb.饰品物件.染色组[1],序列3=bb.饰品物件.染色组[2],序列4=bb.饰品物件.染色组[3]})
