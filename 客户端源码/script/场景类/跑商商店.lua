...
function 场景类_跑商商店:打开(道具组,银子,道具)
	if self.可视 then
		...
		self.资源组 = {
			...
			[7] = 按钮.创建(自适应.创建(12,4,55,20,1,3),0,0,4,true,true,"卖出"),
			[4] = 自适应.创建(3,1,94,18,1,3),
			[5] = 自适应.创建(3,1,94,18,1,3),
			[6] = 自适应.创建(1,1,570,18,1,3,nil,18),
		}
		for n=2,3 do
		   self.资源组[n]:绑定窗口_(23)
		end
...
function 场景类_跑商商店:显示(dt,x,y)
	...
	elseif self.资源组[3]:事件判断() then
		...
		请求服务(1504,{数量=self.数量,商品=self.道具.原始商品})
	elseif self.资源组[7]:事件判断() then
		local sv = tonumber(self.选择物品数量 * self.选中物品单价)
		if sv < 0 then
			引擎.关闭()
		end
		请求服务(1505,{数量=self.选择物品数量,物品=self.选择物品,单价=self.选中物品单价})
	end
	self.焦点 = false
	...
	tp.窗口标题背景_[4]:显示(self.x+82,self.y+2)
	zts1:置字间距(3)
	zts1:显示(self.x+112,self.y+2,"跑商购买")
	zts1:置字间距(0)
	zts:置颜色(4294967295)
	zts:置字间距(15)
	for i=0,3 do
	...
	tp.窗口标题背景_[4]:显示(self.x+369,self.y+2)
	 zts1:显示(self.x+399,self.y+2,"道具行囊")
	 zts1:置字间距(0)
	...
	self.资源组[2]:显示(self.x+553,self.y+2)
