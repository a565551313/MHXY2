...
function 场景类_召唤兽染色:打开(b,l,c)
	...
	if self.可视 then
	    ...
	    self.选中染色=0
	    self.染色数据={}
	    if bb.染色方案~=nil and bb.染色方案~=0 then
	        self.染色数据.id=bb.染色方案
	    	self.染色数据[1]=bb.染色组[1]
	    	self.染色数据[2]=bb.染色组[2]
	    	self.染色数据[3]=bb.染色组[2]
	    end
...
function 场景类_召唤兽染色:置形象(方向)
	if bb ~= nil then
		...
		self.资源组[7] = nil
		if bb.模型=="贪狼" then
		    self.资源组[6] =tp.资源:载入('nccmm.npk',"网易WDF动画","53")
		else
...
function 场景类_召唤兽染色:显示(dt,x,y)
	...
	if self.鼠标 then
		...
		elseif self.资源组[5]:事件判断() then
			if self.选中染色~=0 then
			     ...
			     self.染色数据[3]=染色方案[self.选中染色].方案[3]
			     self:置形象()
			    请求服务(5011,{序列=bb.认证码,序列1=self.染色数据.id,序列2=self.染色数据[1],序列3=self.染色数据[2],序列4=self.染色数据[3]})
			 else
			  	请求服务(5011.1,{序列=bb.认证码})
			end
	...
	if self.资源组[5]:是否选中(x+5,y) then
		tp.提示:自定义(鼠标.x,鼠标.y,"#F/染色需要20个彩果！")
	end
