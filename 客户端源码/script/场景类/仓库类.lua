...
function 场景类_仓库类:显示(dt,x,y)
	...
	self.资源组[12]:更新(x,y)
  	...
  	elseif self.资源组[5]:事件判断() then
  		self.道具类型 = "道具"
        请求服务(80,{文本="获取道具",道具类型=self.道具类型})
	...
	ckzts1:显示(self.x+self.资源组[1].宽度/2-30,self.y+2,"仓  库")
	...
	if self.仓库类型 == "道具" then
		...
		elseif self.资源组[14]:事件判断() then
			请求服务(80,{文本="整理仓库",道具类型=self.道具类型,页数=self.仓库页数})
				...
				if self.物品[i].物品 ~= nil then
					...
					elseif 引擎.鼠标弹起(右键) and (分割名称[1] ~= "" and 分割名称[1] ~= "帮派银票") then
						...
						if self.选择物品 ~= nil then
				 		...
				 		请求服务(80,{文本="存入仓库",页数=self.仓库页数,物品=i,类型=self.道具类型})
				...
				if self.仓库[i].物品 ~= nil then
					...
					if self.仓库[i].事件 and self.鼠标 then
	               	...
	               	elseif 引擎.鼠标弹起(右键) then
					 	请求服务(80,{文本="取出物品",页数=self.仓库页数,物品=i,类型=self.道具类型})
					end
				end
			end
			xx = xx + 1
