...
function 场景类_队伍阵型栏:初始化(根)
	...
	self.阵型位置.地载阵={
		...
		[4]={x=365,y=360},  -- 后排左侧
		[5]={x=493,y=360},  -- 后排右侧
		说明="全能型阵法，全队提升伤害、防御、速度等，全队携带的召唤兽攻击效果提升。",
	}
end
...
function 场景类_队伍阵型栏:打开(数据)
	if self.可视 then
		...
		self.资源组 = {
			...
			[14] = 按钮.创建(自适应.创建(12,4,72,20,1,3),0,0,4,true,true,"选定阵型"),
			[19] = 按钮.创建(自适应.创建(12,4,72,20,1,3),0,0,4,true,true,"进阶阵法"),
			[15] = 资源:载入('登陆资源.wdf',"网易WDF动画",0x0E43E0DF),
			[16] = 资源:载入('登陆资源.wdf',"网易WDF动画",0x57AD4C4E),
			[17] = 自适应.创建(1,1,483,18,1,3,nil,18)
		}
		for n=3,13 do
	      self.资源组[n]=按钮.创建(自适应.创建(12,4,72,20,1,3),0,0,4,true,true,self.阵法名称[n])
		end
		-- 添加八卦阵按钮
		self.资源组[18]=按钮.创建(自适应.创建(12,4,72,20,1,3),0,0,4,true,true,self.阵法名称[18])
		for n=2,14 do
		   self.资源组[n]:绑定窗口_(12)
		end
		self.资源组[18]:绑定窗口_(12)
	    tp.运行时间 = tp.运行时间 + 1
		self.窗口时间 = tp.运行时间
	    self.可视 = true
	    self.阵型数据=数据
...
function 场景类_队伍阵型栏:显示(dt,x,y)
	...
	-- 更新八卦阵按钮
	self.资源组[18]:更新(x,y,w)
	-- 更新进阶阵法按钮
	self.资源组[19]:更新(x,y,not tp.窗口.八卦阵升级界面.可视)
	...
	elseif self.资源组[14]:事件判断() then
	 --[[	if tp.阵型表[self.阵型][2] then
	    ...
	    if self.阵法名称[self.阵型]==nil then
          ...
          请求服务(4009,{名称=self.阵法名称[self.阵型]})
	     end
	elseif self.资源组[19]:事件判断() then
		if self.阵法名称[self.阵型]==nil then
			tp.常规提示:打开("#Y/请先选择一个阵法")
		elseif self.阵法名称[self.阵型] ~= "八卦阵" then
			tp.常规提示:打开("#Y/当前阵法无法进阶，当前仅支持八卦阵进阶")
		elseif self.阵型数据[self.阵法名称[18]]==nil then
			tp.常规提示:打开("#Y/你还没有学会八卦阵阵法")
		elseif not tp.队伍[1] then
			tp.常规提示:打开("#Y/只有队长才可以操作")
		else
			-- 这里打开八卦阵升级界面
			local 八卦阵数据 = {}
			-- 添加八卦阵属性数据
			if tp.队伍[1].高级阵法 and tp.队伍[1].高级阵法.八卦阵 then
			...
			-- 打印八卦阵数据表的详细内容
			print("=== 八卦阵数据 ===")
			for 属性名, 数值 in pairs(八卦阵数据) do
				print(属性名 .. ": " .. tostring(数值))
			end
			print("=== 八卦阵数据结束 ===")
			if 八卦阵升级界面 then
				八卦阵升级界面:打开(tp.队伍[1].高级阵法.八卦阵)
			else
				tp.窗口.八卦阵升级界面:打开(tp.队伍[1].高级阵法.八卦阵)
			end
		end
	end
 for n=3,13 do
	...
	if self.资源组[18]:事件判断() then
		self.阵型 = 18
		self.介绍文本:清空()
        if self.阵型数据[self.阵法名称[18]]==nil then
          self.介绍文本:添加文本("#G/"..self.阵型位置[self.阵法名称[self.阵型]].说明.."#R/(你还不会这个阵法)")
        else
         self.介绍文本:添加文本("#G/"..self.阵型位置[self.阵法名称[self.阵型]].说明)
      	end
	...
	self.资源组[17]:显示(self.x+2,self.y+2)
	self.资源组[16]:显示(self.x + 15,self.y + 85)
	tp.窗口标题背景_[4]:显示(self.x+self.资源组[1].宽度/2-60,self.y+2)
	zts1:置字间距(3)
	zts1:显示(self.x+self.资源组[1].宽度/2-30,self.y+2,"阵型选择")
	zts1:置字间距(0)
	self.资源组[2]:显示(self.x + 466,self.y + 2)
    self.介绍文本:显示(self.x + 280,self.y + 35)
	...
	self.资源组[18]:显示(self.x + 185,self.y + 130)
	self.资源组[14]:显示(self.x + 30,self.y + 288)
	self.资源组[19]:显示(self.x + 100,self.y + 288,nil,true,nil,tp.窗口.八卦阵升级界面.可视,2)
	for n=1,5 do
