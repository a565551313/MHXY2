...
-- 目标坐标,攻击方坐标,挨打距离 算出挨打方附加xy
function 两点算挨打偏移坐标(target,attacker,dist)
  local a = 取两点孤度(target, attacker)
  return 生成XY(-1 * dist * math.cos(a), -1 * dist * math.sin(a))
end

function 弧度算挨打偏移坐标(jd,dist)
  return 生成XY(-1 * dist * math.cos(jd), -1 * dist * math.sin(jd))
end

function 两点算近战攻击坐标(target,attacker,offset)
  local jd = floor(取两点角度(target,attacker))
  local fx = 角度算四方向(jd)
  --print(fx.."fx"..offset.x.."offset.x"..offset.y.."offset.y")
  if fx == 0 then
...
function 两点算保护坐标(target,attacker)
  local jd = floor(取两点角度(target,attacker))
  local fx = 角度算四方向(jd)
  --print(fx.."fx"..offset.x.."offset.x"..offset.y.."offset.y")
  if fx == 0 then
    return 生成XY(target.x + 20,target.y + 20)
  elseif fx ==1 then
    return 生成XY(target.x - 20,target.y + 20)
  elseif fx ==2 then
    return 生成XY(target.x - 20,target.y - 20)
