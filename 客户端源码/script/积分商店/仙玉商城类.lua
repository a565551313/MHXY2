...
-- GB2312安全截取前n个字符（汉字2字节，英文1字节）
local function gb2312_sub(str, n)
    local i, len = 1, 0
    local bytes = #str
    while i <= bytes and len < n do
...
local function utf8_sub(str, start_char, num_chars)
    ...
    while start_char > 1 do
        local c = string.byte(str, start_index)
    ...
    while num_chars > 0 and current_index <= #str do
        local c = string.byte(str, current_index)
        if not c then break end
        if c >= 0 and c <= 127 then
    ...
    return string.sub(str, start_index, current_index - 1)
end
...
function 仙玉商城类:初始化(根)
	...
	self.分类按钮 = {
			...
			[4] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"灵宝商品"),
			...
			[7] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"一级法宝"),
			[8] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"二级法宝"),
			...
			[22] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"房屋装饰"),
			[23] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"庭院装饰"),
			[24] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"一代神兽"),
			[25] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"二代神兽"),
			[26] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"三代神兽"),
			[27] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"四代神兽"),
   ...
   self.积分关闭 =按钮.创建(自适应.创建(18,4,16,16,4,3),0,0,4,24,true)
    ...
    -- 新增GM商城大类按钮
    self.资源组[15] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000052),0,0,4,true,true,"GM商城")
    self.资源组[15]:置文字颜色(0xFFFFA500)
    self.资源组[15]:置偏移(37,4)
    -- 新增GM商城子分类按钮
    self.GM分类按钮 = {}
    for i=1,7 do
        self.GM分类按钮[i] = 按钮.创建(资源:载入('房屋图标.wdf',"网易WDF动画",0x10000047),0,0,3,true,true,"GM"..i)
        self.GM分类按钮[i]:置文字颜色(0xFFFFA500)
        self.GM分类按钮[i]:置偏移(2,8)
    end

    -- 新增一键购买按钮（只在GM商城显示）
    self.一键购买按钮 = 根._按钮.创建(资源:载入('商城.wdf',"网易WDF动画",0x00000001), 0, 0, 4, true, true, "一键购买")
...
function 仙玉商城类:打开(内容)
  if not self.可视 and not 内容 then
    return
  end
  if self.可视 then
    self.可视 = false
...
function 仙玉商城类:刷新货币(内容)
  ...
  -- 更新GM商城剩余限购数据
  if 内容.剩余限购 then
    self.数据.剩余限购 = 内容.剩余限购
  end
  -- 更新GM等级
  if 内容.GM等级 then
    self.数据.GM等级 = 内容.GM等级
  end
...
function 仙玉商城类:刷新(内容)
  if not 内容 then return end
    ...
    for n=1,#内容.商品 do
      self.商品[n]={}
	    ...
	    elseif 内容.商品[n][3]=="神兽" then
	        ...
	        if 内容.商品[n][2]=="灵宝" then
			   self.商品[n].介绍 =self.商品[n].介绍..
								  "\n#W/【使用条件】\n".."#G/"..资源[6].."#G/专用，消耗#G/1/3/7#G点灵元"..
								  "\n#R/【战斗效果】"..
								  "\n#W/"..资源[15]
		   end
    ...
    if self.大类=="银子商城" or  self.大类=="仙玉商城"  or  self.大类=="积分商城"  or  self.大类=="点卡商城" or self.大类=="GM商城" then
        self.结束 = 40
    else
        self.结束 = 24
    end
...
function 仙玉商城类:显示(dt,x,y)
   ...
   elseif self.资源组[8]:事件判断() then
	...
	self.资源组[15]:更新(x,y,self.大类 ~= "GM商城")
    ...
    self.资源组[15]:显示(self.x-41,self.y+267,true,1,nil,self.大类=="GM商城",2)
	...
	if self.大类=="银子商城" then
		...
		for i=22,23 do  -- 这里改为22,23，对应房屋装饰和庭院装饰
			self.分类按钮[i]:更新(x,y,self.状态~=self.分类按钮[i]:取文字())
			if self.分类按钮[i]:事件判断() then
				self.状态=self.分类按钮[i]:取文字()
				请求服务(30,{序列=self.状态,大类=self.大类})
		...
		self.分类按钮[22]:显示(self.x+314,self.y+13,true,1,nil,self.状态==self.分类按钮[22]:取文字(),2)  -- 房屋装饰.
		self.分类按钮[23]:显示(self.x+389,self.y+13,true,1,nil,self.状态==self.分类按钮[23]:取文字(),2)  -- 庭院装饰
		for i=1,5 do
      ...
      elseif self.大类=="积分商城" then
		...
		elseif self.大类=="神兽商城" then
			for i=24,27 do  -- 改为24,25,26,27，对应一代到四代神兽
				self.分类按钮[i]:更新(x,y,self.状态~=self.分类按钮[i]:取文字())
				if self.分类按钮[i]:事件判断() then
					self.状态=self.分类按钮[i]:取文字()
					请求服务(30,{序列=self.状态,大类=self.大类})
			...
			self.分类按钮[24]:显示(self.x+89,self.y+13,true,1,nil,self.状态==self.分类按钮[24]:取文字(),2)  -- 一代神兽
			self.分类按钮[25]:显示(self.x+164,self.y+13,true,1,nil,self.状态==self.分类按钮[25]:取文字(),2)  -- 二代神兽
			self.分类按钮[26]:显示(self.x+239,self.y+13,true,1,nil,self.状态==self.分类按钮[26]:取文字(),2)  -- 三代神兽
			self.分类按钮[27]:显示(self.x+314,self.y+13,true,1,nil,self.状态==self.分类按钮[27]:取文字(),2)  -- 四代神兽
			for i=1,3 do
			   self.黑线:显示(self.x+89+75*i,self.y+23)
			end
       elseif self.大类=="GM商城" then
            for i=1,7 do
                self.GM分类按钮[i]:更新(x,y,self.状态 ~= self.GM分类按钮[i]:取文字())
                if self.GM分类按钮[i]:事件判断() then
                    self.状态 = self.GM分类按钮[i]:取文字()
                    请求服务(30,{序列=self.状态,大类=self.大类})
                end
            end
            -- 子分类按钮横向排列
            for i=1,7 do
                self.GM分类按钮[i]:显示(self.x+89+(i-1)*75,self.y+13,true,1,nil,self.状态==self.GM分类按钮[i]:取文字(),2)
            end
            for i=1,6 do
                self.黑线:显示(self.x+89+75*i,self.y+23)
            end
    ...
    -- 一键购买按钮显示与事件
    if self.大类=="GM商城" then
        self.一键购买按钮:显示(self.x+320, self.y+460)
        self.一键购买按钮:更新(x,y)
        if self.一键购买按钮:事件判断() then
            -- 先验证GM权限
            local gm_type = tonumber(string.match(self.状态, "GM(%d)"))
            if not gm_type or (self.数据.GM等级 or 0) < gm_type then
                tp.常规提示:打开("#Y你没有权限购买该GM商品！")
                return
            end
            tp.常规提示:打开("#Y/正在一键购买...")
            for i=self.开始,self.结束 do
                local num = self.数据.剩余限购 and self.数据.剩余限购[i] or 0
                if num and num > 0 then
                    请求服务(30.2,{序列=self.状态,序列1=i,数量=num,大类=self.大类})
                end
            end
            tp.常规提示:打开("#Y/已自动购买所有可买商品")
        end
    end
			local xx = 0
			local yy = 0
			for i=self.开始,self.结束 do
		        if self.商品[i]~=nil and  self.商品[i].小动画~=nil then
		            ...
		            -- 商品名显示（下方居中，最大4字符，超出截断，白色黑描边）
		            local 名称 = self.商品[i].名称 or ""
		            if #名称 > 4 then
		                名称 = gb2312_sub(名称, 4)
		            end
		            local 字体 = tp.字体表.描边字体
		            字体:置颜色(0xFFFFFFFF):置描边颜色(0xFF000000)
		            local 文本宽度 = 字体:取宽度(名称)
		            local 动画x = self.x + xx * 65 + 101
		            local 动画y = self.y + yy * 70 + 107
		            local 名称x = 动画x + (48 - 文本宽度 + 5)
		            local 名称y = 动画y + 48 + 5 - 30 + 10
		            字体:显示(名称x, 名称y, 名称)
		            -- 只在GM商城显示剩余限购数字，且坐标与商品动画完全一致
		            if self.大类=="GM商城" and self.数据.剩余限购 and self.数据.剩余限购[i] then
		              local num = self.数据.剩余限购[i]
		              if num >= 0 then
		                tp.字体表.描边字体:置颜色(0xFFFFFFFF):置描边颜色(0xFF000000):显示(self.x + xx * 65+101,self.y + yy * 70 +107,""..num)
		              end
		            end
		        end
		        ------------------------------------
...
-----------------------------------------
			...
			if self.道具 ~= nil then
				zts:置颜色(取商城金钱颜色(self.单价)):显示(self.x + 490,self.y + 385,self.单价)
				zts:置颜色(0xFFFFFFFF):显示(self.x + 170,self.y + 385,self.道具.名称)
				self.输入框:置坐标(self.x-90,self.y+48)
				if self.输入框:取文本() == "" then
					self.输入框:置文本(1)
				end
				if self.大类=="GM商城" and self.数据.剩余限购 and self.选中>0 then
                    local max_num = self.数据.剩余限购[self.选中]
                    -- 只有当剩余限购数量大于0时才限制输入
                    if max_num and max_num > 0 and tonumber(self.输入框:取文本()) > max_num then
                        self.输入框:置文本(max_num)
                    end
                end
				...
				self.总额=tonumber(self.单价*self.数量)
				zts:置颜色(取商城金钱颜色(self.总额)):显示(self.x + 490,self.y + 420,self.总额)
			end
		   self.控件类:更新(dt,x,y)
       ...
       self.覆盖界面:显示(self.x+95,self.y+100)
		 ...
		 for i=self.开始,self.结束 do
		        if self.商品[i]~=nil and  self.商品[i].小动画~=nil then
		            ...
		            字体:置颜色(0xFFFFFFFF):置描边颜色(0xFF000000)
		            local 文本宽度 = 字体:取宽度(名称)
		            local 动画x = self.x + xx * 65 + 367
		            local 动画y = self.y + yy * 63 + 106
		            local 名称x = 动画x + (48 - 文本宽度 + 5)
		            local 名称y = 动画y + 48 + 5 - 30 + 10
		            字体:显示(名称x, 名称y, 名称)
		        end
...
-----------------------------------------
			...
			if self.道具 ~= nil then
				zts:置颜色(取商城金钱颜色(self.单价)):显示(self.x + 450,self.y + 390,self.单价)
				...
				self.数量 = tonumber(self.输入框:取文本())
				self.总额=tonumber(self.单价*self.数量)
				zts:置颜色(取商城金钱颜色(self.总额)):显示(self.x + 635,self.y + 425,self.总额)
			end
		 ...
		 for i=self.开始,self.结束 do
		        if self.商品[i]~=nil and  self.商品[i].小动画~=nil then
		            ...
		            字体:置颜色(0xFFFFFFFF):置描边颜色(0xFF000000)
		            local 文本宽度 = 字体:取宽度(名称)
		            local 动画x = self.x + xx * 65 + 364
		            local 动画y = self.y + yy * 63 + 105
		            local 名称x = 动画x + (48 - 文本宽度 + 5)
		            local 名称y = 动画y + 48 + 5 - 30 + 10
		            字体:显示(名称x, 名称y, 名称)
		        end
...
-----------------------------------------
    ...
    zts:置颜色(取商城金钱颜色(self.数据.银子)):显示(self.x + 300,self.y -28,self.数据.银子)
    if self.大类== "点卡商城" then
	   zts:置颜色(取商城金钱颜色(self.数据.点卡)):显示(self.x + 125,self.y -28,self.数据.点卡)
	elseif self.大类== "积分商城" then
		zts:置颜色(取商城金钱颜色(self.数据[self.状态])):显示(self.x + 125,self.y -28,self.数据[self.状态])
	else
	    zts:置颜色(取商城金钱颜色(self.数据.仙玉)):显示(self.x + 125,self.y -28,self.数据.仙玉)
	end
 ...
 if  self.积分显示 then
		...
		self.积分显示=false
	 elseif self.积分界面:是否选中(x,y) then
	 	    self.焦点1=true
	 	 if 引擎:鼠标弹起(右键)  then
	 	    self.积分显示=true
           end
 ...
 for i=8,14 do
  ...
  -- 2. 商品渲染时显示剩余限购
  if self.大类=="GM商城" and self.GM等级 then
    -- GM等级显示位置右移，字体与分类一致，数字也用排行字体
    tp.字体表.排行字体:显示(self.x+430,self.y-40,string.format("GM等级:%d", self.GM等级),红色)
	tp.字体表.排行字体:显示(self.x+430,self.y-20,string.format("GM余币:%d", self.数据.GM币),红色)
	--tp.字体表.排行字体:显示(self.x+450,self.y-30,string.format("GM币:%d", self.数据.GM币),红色)
  end
