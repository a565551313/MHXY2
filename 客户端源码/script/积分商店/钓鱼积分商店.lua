...
function 积分商店_钓鱼积分商店:打开(道具组,名称)
	if self.可视 then
		for i=1,25 do
			self.商品[i]:置物品(nil)
		end
...
function 积分商店_钓鱼积分商店:加载数据()	-- body
	local 资源 = tp.资源
	local 按钮 = tp._按钮
	local 自适应 = tp._自适应
	self.资源组 = {
...
function 积分商店_钓鱼积分商店:显示(dt,x,y)
	self.资源组[2]:更新(x,y)
	...
	elseif self.资源组[3]:事件判断() then
		...
		请求服务(1503.1,{数量=self.数量,商品=self.道具.原始商品})
	end
	self.焦点 = false
	self.资源组[1]:显示(self.x,self.y)
	self.资源组[6]:显示(self.x+2,self.y+2)
	-- tp.横排花纹背景_:置区域(0,0,248,13)
	-- tp.横排花纹背景_:显示(self.x+14,self.y+293)
    tp.窗口标题背景_[4]:显示(self.x+self.资源组[1].宽度/2-60,self.y+2)
	 zts1:置字间距(3)
	  if self.商店名称==nil then
		 zts1:显示(self.x+self.资源组[1].宽度/2-30,self.y+2,"钓鱼积分")
	  else
	  	 zts1:显示(self.x+self.资源组[1].宽度/2-30,self.y+2,self.商店名称)
      end
	 zts1:置字间距(0)
	zts:置颜色(4294967295)
	zts:置字间距(15)
	...
	self.资源组[2]:显示(self.x+251,self.y+2)
	...
	zts:置颜色(tos(tp.钓鱼积分))
	zts:显示(self.x + 78,self.y + 393,tp.钓鱼积分)
	self.控件类:更新(dt,x,y)
...
function 积分商店_钓鱼积分商店:检查点(x,y)
	if self.资源组 ~= nil and self.资源组[1]:是否选中(x,y)  then
		return true
	end
end

function 积分商店_钓鱼积分商店:初始移动(x,y)
	tp.运行时间 = tp.运行时间 + 1
