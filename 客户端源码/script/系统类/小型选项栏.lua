...
-- @Last Modified time: 2023-09-21 22:11:58
--======================================================================--
...
function 系统类_小型选项栏:打开(选项,背景)
	if self.可视 then
		...
		if #选项 > self.列表.最大显示量 then
		    ...
		    self.滑块 = tp._滑块.创建(tp._自适应.创建(11,4,7,40,2,3,nil),4,14,self.高度-42,2)
		    self.允许加入 = true
		else
...
function 系统类_小型选项栏:显示(sx,sy,x,y,sb)
	...
	if self.允许加入 then
	    ...
	    self.滑块:显示(sx+self.背景.宽度-9,sy+1,x,y,sb,true)
	    if self.滑块.接触  then
	    	self.列表.起始 = min(ceil(self.列表.最大显示量*self.滑块:取百分比())+1,self.列表.最大显示量-1)
	    end
	end
	if 引擎.取鼠标滚轮()==1 and self.列表.起始 >1 and self.背景:是否选中(x,y) then
		self.列表.起始 = self.列表.起始 -1
	elseif 引擎.取鼠标滚轮()==-1 and self.列表.起始 < self.列表.最大显示量 and self.背景:是否选中(x,y) then
		self.列表.起始 = self.列表.起始 + 1
	end
