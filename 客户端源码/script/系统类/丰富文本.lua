...
function 丰富文本类:初始化(...)
	...
	self.行间距 		= 1
	self.元素表 		= {P=0xFFFF1493,S=-419495936,X=ARGB(250,250,180,220),W=0xFFFFFFFF,H=0xFF000000,Y=0xFFFFFF00,L=-16776961,R=4294901760,G=4278255360,S=-16466190,Z=-16711681,N=-16777216,C=ARGB(255,125,125,125)}
	self.显示表 		= {高度 = 0,宽度 = 0}
	--亮金黄色：FFFAFAD2
	...
	self.回调 		= {}
	self.回调包围盒 = require("gge包围盒2")()
	self.寻路包围盒 = require("gge包围盒2")()
	...
	self.滚动值 	= 0 --显示偏移
	self.加入 = 0
	self.可点击坐标  = {x=0,y=0,x1=0,y1=0,cs=0}
	for n=0,190 do
...
function 丰富文本类:_生成样式(段数据,内容)
	...
	if 段数据.下划线 then
		...
		段数据.内容=string.gsub(段数据.内容, "/", "")
		local 临时文本 = 分割文本(段数据.内容,"|")

    if 临时文本[1] ~=nil and 临时文本[1]~=""   then
		段数据.地图=临时文本[1]
				-- 段数据.NPC=临时文本[2]
				-- 段数据.坐标x=临时文本[3]
			 -- 	段数据.坐标y=临时文本[4]
...
function 丰富文本类:_解析(数据,id)
	local x 		= 0
	local 行数据  	= {x = 0,宽度 = 0,高度 = self.文字高度,内容 = '',id=id}--行初始化
	if 数据:find('#') then
...
function 丰富文本类:添加文本(数据,物品数据,宽度)
	if 数据 == nil then
	    return
	end
	insert(self.添加记录, 数据)
	self.物品数据[#self.显示表+1]=物品数据
	数据 = 数据:gsub('##','#井\0')
	if 数据:sub(1,1)~='#' then
		数据 = '#'..self.默认元素 .. 数据
	end
	...
	if (self.行数量 > 0) then
		...
		for i=self.起始值,self.目标值,self.递增值 do
				...
				if 行数据~=nil then
					local 包围盒宽度 = 0
					local 值= ""
				 for k,v in ipairs(行数据) do
					if v.内容~="" and v.内容~=nil and type(v.内容)~='table' and k>0 then
						值 	= v.内容:match("(%［.*%］)")
						if 值~= nil then
							if next(self.物品数据) ~= 0 then
							v.回调 = true
							v.回调数据 = self.物品数据[i]
							end
						end
			     ...
			     for k,v in ipairs(行数据) do
					...
					local 前缀宽度 = 0
					if v.回调 then
						if string.find(显示内容,值,1)~=nil then
							local 临时文本 = 分割文本(显示内容,"［")
							前缀宽度 = self.文字:取宽度(临时文本[1])
						end
						包围盒宽度 = self.文字:取宽度(值)
						self.回调包围盒:置坐标宽高(显示xx+前缀宽度 ,显示yy,包围盒宽度,v.高度)
						if self.回调包围盒:检查点(鼠标.x,鼠标.y) then
							超链接碰撞=true
							tp.窗口.消息框.碰撞数量 =  1
						    ...
						    if  引擎.鼠标弹起(左键) then
                                if v.回调数据==nil then
                                tp.常规提示:打开("#Y/没获取到回调数据")
                                return
                                end
						    	 if v.回调数据.寿命 then
						    	   tp.窗口.召唤兽查看栏:打开(v.回调数据)
						    	 else
						    	   物品链接展示 = true
						    	   物品链接数据 = v.回调数据
						    	 end
					...
					if v.下划线 then
						...
						--self:置可选区域(显示x,显示y0+v.高度-20,显示x+v.宽度,显示y0+v.高度+20)
						包围盒宽度 = self.文字:取宽度(值)
						self.寻路包围盒:置坐标宽高(显示x,显示y0+v.高度-20,v.宽度,v.高度+20)
						if self.寻路包围盒:检查点(鼠标.x,鼠标.y) and not tp.消息栏焦点 and not tp.选中UI then

							if 任务坐标~=nil then
						       for k,n in pairs(任务坐标) do
		 			            if 任务坐标[k].名称~=nil and  任务坐标[k].名称==v.内容 then
                                     self.坐标更新 = 任务坐标[k]
					 			    	break
				 			        end
								end
							end
                          if  self.坐标更新 ~= nil and self.坐标更新.地图~=nil  then
							tp.鼠标.置鼠标("事件")
							tp.提示:自定义(鼠标.x,鼠标.y+30,"#Y"..self.坐标更新.名称.."在"..取地图名称(self.坐标更新.地图+0).."("..self.坐标更新.x..","..self.坐标更新.y..")")
							if 引擎.鼠标按下(左键) then
								请求服务(41,self.坐标更新)
								self.坐标更新 ={}
							end
					...
					if g ~= nil and a then
					if	颜色 ~=-419495936 and 颜色 ~= ARGB(250,250,180,220) and 颜色 ~= 0xFFFFFFFF and 颜色 ~=0xFF000000 and 颜色 ~=0xFFFFFF00 and 颜色 ~=-16776961 and 颜色 ~=4294901760 and 颜色 ~=4278255360 and 颜色 ~=-16466190 and 颜色 ~=-16711681 and 颜色 ~=-16777216 and 颜色 ~=ARGB(255,125,125,125) then
						颜色 = a
					end
						g = nil
					end
					...
					if v.类型 == 0 then
						文字:置颜色(颜色)
						文字:显示(显示x+2 ,显示y0+1,显示内容)
						if 超链接碰撞 then
							文字:置颜色(0xFF00FF00):显示(显示x+2+前缀宽度 ,显示y0+1,值)
						end
					elseif v.类型 == 1 then
						if 显示内容.帧数 == 1 then
							显示内容:显示(显示x+3 ,显示y0+19)
						else
							local sj = max(显示内容.高度,26)
							local yy=0
							 if v.内容.标识  then
							 	for n=121,190 do     -----------靓号位置调
								 	if tostring(n)==v.内容.标识 then
								 		 yy=-22
								 		 break
								    end
								end
						     end
							显示内容:显示(显示x+4 ,显示y0+sj+yy)
						end
					end
				 end
				end
...
function 丰富文本类:置可选区域(x,y,x1,y1)
	if self.可点击坐标.x == 0 then
		self.可点击坐标.x  = x
		self.可点击坐标.y  = y
		self.可点击坐标.x1  = x1
		self.可点击坐标.y1  = y1
